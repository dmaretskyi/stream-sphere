...
const searchOnPirateBay = async (query: string) => {
  ...
  const byImdbId = results.reduce<Record<string, SearchResult[]>>(
    (acc, result) => {
  ...
  return await Promise.all(
    Object.entries(byImdbId).map(async ([imdbId, results]) => {
      const imdbData = !imdbId.startsWith('nil')
        ? await fetchTitleData(imdbId)
        : undefined;
      ...
      return {
        id: !imdbId.startsWith('nil')
          ? `imdb:${imdbId}`
          : `piratebay:${results[0].id}`,
        info: {
