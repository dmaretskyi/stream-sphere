...
export const importBackup = async (db: Database) => {
  ...
  const dir =
    '/tmp/stream-sphere-mal-import9xHs3O' ??
    (await mkdtemp('/tmp/stream-sphere-mal-import'));
  const process = await spawn(
    `git clone https://github.com/bal-mackup/mal-backup ${dir}`,
    ...
    await parallel.each(
      ...
      async (file) => {
        const data = JSON.parse(
          await readFile(join(dir, subdir, file), { encoding: 'utf-8' })
        );
        await db.run(
          sql`INSERT INTO mal_backup(id, data) VALUES(${keyprefix + file.slice(0, -'.json'.length)}, ${JSON.stringify(data)})`
        );
        done++;
...
export const searchMalBackup = async (db: Database, query: string) => {
  const entry = await db.all(
    sql`SELECT * FROM mal_backup WHERE data MATCH ${query}`
  );
